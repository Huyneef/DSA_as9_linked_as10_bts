...
tuple<List, List, List> partition(List& l, int pivot) {
    ...
    while (curr != NULL) {
        if (curr->val < pivot) {
            add_tail(l1, curr->val);
        }
        else if (curr->val == pivot) {
            add_tail(l2, curr->val);
        }
        else {
            add_tail(l3, curr->val);
        }
        curr = curr->next;
    }
...
void join(List& l, List& sublist) {
    List ll;
    list_initializing(ll);
    if (sublist.head != NULL) {
        if (ll.head == NULL) {
            ll.head = sublist.head;
            ll.tail = sublist.tail;
        }
        else {
            ll.tail->next = sublist.head;
            ll.tail = sublist.tail;
        }
    }
